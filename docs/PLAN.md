...
| 2   | Recommendations are a **deterministic rules engine** over the user's real stats, labelled in-UI as _"Rule-based insights"_. No OpenAI dependency |
...
_Verify:_ `git status` clean; `rg` finds no remaining references to any removed path.
...
_Verify:_ `npm ci` succeeds **with no env-var workarounds**; `npm audit` high/critical count reported before and after; `node_modules` size before and after.
...
_Verify:_ `npm run lint`, `npm run format:check`, `npm run typecheck` all exit 0 — the first time any of them ever has.
...
_Verify:_ `npm run typecheck` exits 0; `npm run build` exits 0.
...
_Verify:_ integration tests (item 13) run green against **both** implementations.
...
_Verify:_ `vite build` transforms **hundreds** of modules, not 2; app boots and renders locally.
...
_Verify:_ integration tests assert 401 on missing auth and 403 on cross-user access — both currently pass through.
...
_Verify:_ manual click-through of every control; E2E specs in item 14.
...
_Verify:_ Playwright screenshots at 360/768/1280; keyboard-only traversal of every page; axe-core accessibility scan in the E2E suite; console asserted clean.
...
_Verify:_ E2E test that lands on `/login` and reaches a populated dashboard in one click, with no credentials typed.
...
_Verify:_ `npm test` green; coverage reported honestly (no invented threshold I can't hit); `docs/TESTING.md` records what's covered, what passed, and what I couldn't test and why.
...
_Verify:_ production build succeeds — the first time in this project's history — and the deployment reaches `READY`, not `ERROR`.

**15. GitHub Actions CI**
`.github/workflows/ci.yml`: install → lint → typecheck → test → build on push and PR.
_Verify:_ green check on the new PR.
